...
#include "isr.h"
//定时器周期标志位//
int16 TImer_counter_1=0;
int16 TImer_counter_2=0;
int16 cnt1=0;
uint8 clear_flag = 0;
...
IFX_INTERRUPT(cc60_pit_ch0_isr, CCU6_0_CH0_INT_VECTAB_NUM, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    if(TImer_counter_1%2 == 0)
    {
        ...
        if(cargo == 2)//发车标志位判断
        {
            ...
            if(!stop_flag)
            {
                Mid_angle_Y = expect_angle;//期望角度改变
                Expect_Velocity = Increase_Velocity(2.1,Expect_Velocity,Velocity_limit);//期望速度增加
                differ = Increase_Differ(0.4,differ,servo);
            }
            else
            {
                Expect_Velocity = Increase_Velocity(2,Expect_Velocity,0);//期望速度增加
                differ = Increase_Differ(0.4,differ,0);                     //18ms
            }
        }
    }
    ...
    Med_Avg_Filter_imu963ra_acc();//中位值滤波获取加速度信息
    Move_Avg_Filter_imu963ra_gyro();//滑动平均滤波获取角速度值
    MahonyAHRSupdateIMU(-Gyro_X,-Gyro_Z,-Gyro_Y,ax,az,ay);//四元数解算姿态

    if(cnt1%10==0)//速度环处理
    {
        clear_flag = 1;
        Encoder_Deal();//编码采样
        LR_Balance_Speed_Deal();
        FB_Balance_Speed_Deal();
    }

    if(cnt1%2==0)//角度环处理
    {
        Dynamic_Deal_Angle();
        Servo_Angle_Deal();//转向环处理
        LR_Balance_Angle_Deal();
        FB_Balance_Angle_Deal();
    }

    if(KEY_blance && Blance_flag)//角速度处理
    {
        Servo_Gyro_Deal();
        LR_Balance_Gyro_Deal();
    }
    else
    {
        Left_Stop;
    ...
    if(KEY_blance && Blance_flag)//角速度处理
        FB_Balance_Gyro_Deal();
    else
        DC_Out_Pwm(Go,0);

    if(clear_flag)//定时器计数位清零
    {
        clear_flag = 0;
        cnt1 = 0;
    }
